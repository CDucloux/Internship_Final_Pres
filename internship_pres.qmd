...
:::{.notes}
je vais commencer par introduire EDF et le CNEPE
:::
...
:::{.notes}
Ensuite je vais expliciter ce que j'ai fait dans le cadre de ma première et de ma deuxième mission,
:::
...
:::{.notes}
Je vais continuer cette introduction assez générale en utilisant quelques chiffres clés concernant le groupe.

Avec un chiffre d’affaires de 140 milliards d’euros et un effectif de 180000 personnes en 2023, EDF est un des grands leaders mondiaux de l’énergie, et c'est particulièrement vrai en France et en Europe. 
:::
...
Dans le mix eléctrique français on voit surtout l'importance considérable de l'énergie d'origine nucléaire.

Au delà de ça on voit en fait que le mix électrique en France pour EDF il est de facto presque entièrement décarbonné, grâce au nucléaire et à l'hydroélectrique.
:::
...
La version que j'ai développée d'HPConnect, va beaucoup plus loin que celle initialement pensée parce qu'elle intègre notamment :
...
De manière analogue à ce qu'on peut trouver dans l'aviation avec les simulateurs de vol, dans le nucléaire vous avez aussi des simulateurs dont le but va être de reproduire au mieux la tranche et les phénomènes physiques, en modélisant tous les sytèmes et la logique de contrôle commande.

Le but d'un simulateur à première vue, c'est faire de la formation et c'est vrai, mais pas que, ça peut aussi être utilisé dans le cadre de vérification de modifications sur l'installation et de rédaction de notes techniques - plus dans le cadre du groupe, etc etc.
...
Pour la petite histoire ça s'appelle ViZiR parce que c'était un acronyme de Visualisation sur R, mais voilà, les applis sur R shiny comme celle que j'ai pu vous présenter dans la partie précédente c'est bien MAIS ça gère très mal un grand nombre d'utilisateurs en simultanné (or y'a plusieurs entités d'EDF qui manifestent leur désir d'utiliser ViZiR donc plusieurs utilisateurs en simultanné c'est clairement possible).

Aussi, si jamais y'a pas de gestion d'erreur appropriée, ça déconnecte automatiquement les utilisateurs du serveur. Niveau stabilité on a vu mieux.

Et surtout, R c'est pas un langage qui a le vent en poupe chez EDF donc y'a pas forcément d'infrastructure adaptée où on peut déployer facilement des applis shiny, et ça ça compte.

***

Maintenant la question après cet aparté c'est qu'est ce que permet ViZiR :

- Un temps de paramétrage qui par rapport à Excel, est divisé par 10
- Ensuite, grâce aux bases de données de simulateur, on peut récupérer les libellés associés aux capteurs, les unités, etc.
...
- Changer le type de tracé, là au lieu de lignes on a seulement les points, ça peut être utile pour identifier les périodes d'acquisition des capteurs.
...
:::{.notes}
Travailler chez EDF ça a été une expérience professionnelle très agréable et enrichissante, j'ai eu l'avantage de travailler sur 2 sujets différents :
...
Ca m'a permis de mobiliser beaucoup de compétences techniques qui sont enseignées au Mécen, mais surtout aussi d'apprendre, et je le dis avec humilité, beaucoup de choses spécifiques au domaine du nucléaire puisque j'étais en contact direct avec des ingénieurs du CNEPE, et j'ai aussi eu l'occasion d'aller sur le site de production de Dampierre, on voit d'ailleurs à gauche l'aéroréfrigérant de la tranche à l'arrêt, qu'on a pu visiter.

Une des choses que j'ai apprécié c'est le fait que maintenant ce que j'ai développé c'est utilisé et valorisé. C'est pas quelque chose d'expérimental qui reste dans son coin, y'a un véritable impact. Pour garantir le mantien de la solution dans le futur je me suis aussi beaucoup appuyé sur le groupe SID, que Guillaume vous présentera par la suite et je me suis aussi mis en contact avec l'équipe Data Science coté Edvance (une filiale d'EDF),  qui lance beaucoup de choses sur ces sujets.  
:::
